...
mod tests {
    ...
    const APPLE_KEYS: &[&str] = &["APPLE_CLIENT_ID", "APPLE_AUTH_URL", "APPLE_TOKEN_URL"];
    ...
    async fn test_handle_apple_oauth_missing_env_vars() {
        ...
        remove_env_vars(APPLE_KEYS);

        let pool: Option<PgPool> = None;
        let auth_code = "test_auth_code".to_string();

        let result = handle_apple_oauth(pool.as_ref(), auth_code).await;
    ...
    async fn test_handle_github_oauth_missing_env_vars() {
        // Clear any existing environment variables that might interfere
        remove_env_vars(GITHUB_KEYS);

        let pool: Option<PgPool> = None;
        let auth_code = "test_auth_code".to_string();

        let result = handle_github_oauth(pool.as_ref(), auth_code).await;
    ...
    async fn test_handle_apple_oauth_invalid_auth_url() {
        set_env_vars(&[
            ...
            ("APPLE_AUTH_URL", "invalid_url"),
            ("APPLE_TOKEN_URL", "https://valid.token.url"),
        ]);
    ...
    async fn test_handle_apple_oauth_empty_auth_code() {
        set_env_vars(&[
            ("APPLE_CLIENT_ID", "test_client_id"),
            ("APPLE_AUTH_URL", "https://valid.auth.url"),
            ("APPLE_TOKEN_URL", "https://valid.token.url"),
        ]);
    ...
    async fn test_handle_github_oauth_empty_auth_code() {
        set_env_vars(&[
            ("GITHUB_CLIENT_ID", "test_client_id"),
            ("GITHUB_CLIENT_SECRET", "test_client_secret"),
            ("GITHUB_AUTH_URL", "https://valid.auth.url"),
            ("GITHUB_TOKEN_URL", "https://valid.token.url"),
        ]);
    ...
    fn test_authorization_code_creation() {
        ...
        let empty_code = oauth2::AuthorizationCode::new(String::new());
    ...
    async fn test_handle_apple_oauth_invalid_token_url() {
        set_env_vars(&[
            ("APPLE_CLIENT_ID", "test_client_id"),
            ("APPLE_AUTH_URL", "https://valid.auth.url"),
            ("APPLE_TOKEN_URL", "invalid_token_url"),
        ]);
    ...
    async fn test_handle_github_oauth_invalid_token_url() {
        set_env_vars(&[
            ...
            ("GITHUB_CLIENT_SECRET", "test_client_secret"),
            ("GITHUB_AUTH_URL", "https://valid.auth.url"),
            ("GITHUB_TOKEN_URL", "invalid_token_url"),
        ]);
    ...
    async fn test_handle_apple_oauth_missing_auth_url() {
        ...
        remove_env_vars(&["APPLE_AUTH_URL"]);
        ...
        // Clean up
        remove_env_vars(&["APPLE_CLIENT_ID", "APPLE_TOKEN_URL"]);
    }
    ...
    async fn test_handle_github_oauth_missing_client_secret() {
        ...
        remove_env_vars(&["GITHUB_CLIENT_SECRET"]);
        ...
        // Clean up
        remove_env_vars(&["GITHUB_CLIENT_ID", "GITHUB_AUTH_URL", "GITHUB_TOKEN_URL"]);
    }
    ...
    fn test_oauth_client_secret_creation() {
        ...
        let empty_secret = oauth2::ClientSecret::new(String::new());
    ...
    fn test_oauth_url_edge_cases() {
        ...
        for valid_url in valid_urls {
            let auth_url_result = oauth2::AuthUrl::new(valid_url.to_string());
            assert!(auth_url_result.is_ok(), "URL should be valid: {valid_url}");

            let token_url_result = oauth2::TokenUrl::new(valid_url.to_string());
            assert!(token_url_result.is_ok(), "URL should be valid: {valid_url}");
        }
    }
